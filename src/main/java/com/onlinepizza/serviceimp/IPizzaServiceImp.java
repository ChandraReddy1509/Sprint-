...
public class IPizzaServiceImp implements IPizzaService { // all methods implemented
	...
	//1
	public Pizza viewPizzaById(Integer pizzaId)throws PizzaManagementException {
		if(pizzaRepository.existsById(pizzaId)==false) {
			throw new PizzaManagementException("Pizza not exist");
		}else
		return pizzaRepository.findById(pizzaId).get();
	}

	//2
	public List<Pizza> viewPizzaByPizzaType(String pizzaType) throws PizzaManagementException {//2
		...
		if(!pizzaByType.isEmpty()) {
			...
			throw new PizzaManagementException("Pizza type doesnot exist");
		}
//			return null;
	}
	...
	public Toppings viewToppingByID(Integer toppingsID) throws PizzaManagementException  {
		if(toppingsRepository.existsById(toppingsID)==false) {
			throw new PizzaManagementException("Invalid id");
		}
		else {
